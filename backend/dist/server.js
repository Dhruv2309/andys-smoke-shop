...
async function getOrCreateCart(userId) {
    const existing = await db.query(`SELECT id FROM carts WHERE user_id = $1 AND status = 'active' ORDER BY updated_at DESC LIMIT 1`, [userId]);
...
async function start() {
    ...
    const registerSchema = zod_1.z.object({
        ...
        dateOfBirth: zod_1.z.string().regex(/^\d{4}-\d{2}-\d{2}$/, 'Format: YYYY-MM-DD')
    });
    app.post('/api/auth/register', async (request, reply) => {
        ...
        if (age < 21)
            return reply.status(400).send({ error: 'You must be 21 or older to register.' });
        const passwordHash = await bcryptjs_1.default.hash(password, 12);
        try {
            const result = await db.query(`INSERT INTO users (email, password_hash, first_name, last_name, date_of_birth)
         VALUES ($1, $2, $3, $4, $5)
         RETURNING id, email, first_name, last_name`, [email, passwordHash, firstName, lastName, dateOfBirth]);
    ...
    app.post('/api/auth/login', async (request, reply) => {
        ...
        if (!body.success)
            return reply.status(400).send({ error: body.error.format() });
        const result = await db.query(`SELECT id, email, password_hash, first_name, last_name FROM users WHERE email = $1`, [body.data.email]);
        const u = result.rows[0];
        if (!u || !(await bcryptjs_1.default.compare(body.data.password, u.password_hash))) {
    ...
    app.get('/api/auth/me', { onRequest: [authenticate] }, async (request, reply) => {
        ...
        const result = await db.query(`SELECT id, email, first_name, last_name, age_verified FROM users WHERE id = $1`, [request.user.userId]);
        if (!result.rows[0])
    ...
    app.get('/api/cart', { onRequest: [authenticate] }, async (request, reply) => {
        ...
        const { rows } = await db.query(`SELECT product_id, quantity FROM cart_items WHERE cart_id = $1`, [cartId]);
        const items = rows
            .map((row) => {
    ...
    app.post('/api/cart/items', { onRequest: [authenticate] }, async (request, reply) => {
       ...
       DO UPDATE SET quantity = cart_items.quantity + EXCLUDED.quantity`, [cartId, body.data.productId, body.data.quantity]);
        await db.query(`UPDATE carts SET updated_at = NOW() WHERE id = $1`, [cartId]);
        return { success: true };
    });
    // ── Cart: Remove Item ─────────────────────────────────────────────────────
    app.delete('/api/cart/items/:productId', { onRequest: [authenticate] }, async (request, reply) => {
        if (!db)
            return reply.status(503).send({ error: 'Database not configured' });
        const cartId = await getOrCreateCart(request.user.userId);
    ...
    app.get('/api/payments/methods', { onRequest: [authenticate] }, async (request, reply) => {
        ...
        const { rows: dbRows } = await db.query('SELECT stripe_payment_method_id, is_default FROM payment_methods WHERE user_id = $1', [request.user.userId]);
        ...
        for (const pm of stripeList.data) {
            await db.query(`INSERT INTO payment_methods (user_id, stripe_payment_method_id, type, brand, last4, exp_month, exp_year)
         VALUES ($1, $2, $3, $4, $5, $6, $7)
         ON CONFLICT (stripe_payment_method_id) DO UPDATE
           SET brand = EXCLUDED.brand, last4 = EXCLUDED.last4,
               exp_month = EXCLUDED.exp_month, exp_year = EXCLUDED.exp_year`, [request.user.userId, pm.id, pm.type, pm.card?.brand, pm.card?.last4, pm.card?.exp_month, pm.card?.exp_year]);
        }
    ...
    app.delete('/api/payments/methods/:id', { onRequest: [authenticate] }, async (request, reply) => {
        if (!stripe || !db)
            return reply.status(503).send({ error: 'Service not configured' });
        await stripe.paymentMethods.detach(request.params.id);
        await db.query('DELETE FROM payment_methods WHERE stripe_payment_method_id = $1 AND user_id = $2', [request.params.id, request.user.userId]);
        return { success: true };
    });
    // ── Payment Methods: Set Default ──────────────────────────────────────────
    app.put('/api/payments/methods/:id/default', { onRequest: [authenticate] }, async (request, reply) => {
        if (!db)
            return reply.status(503).send({ error: 'Database not configured' });
        await db.query('UPDATE payment_methods SET is_default = false WHERE user_id = $1', [request.user.userId]);
        await db.query('UPDATE payment_methods SET is_default = true WHERE stripe_payment_method_id = $1 AND user_id = $2', [request.params.id, request.user.userId]);
        return { success: true };
    });
    ...
    app.post('/api/age-verification/session', { onRequest: [authenticate] }, async (request, reply) => {
        ...
        await db.query(`INSERT INTO age_verifications (user_id, stripe_session_id) VALUES ($1, $2)
       ON CONFLICT (stripe_session_id) DO NOTHING`, [request.user.userId, session.id]);
        ...
        if (event.type === 'identity.verification_session.verified') {
            ...
            if (userId && db && dob?.year && dob?.month && dob?.day) {
                ...
                if (age >= 21) {
                    ...
                    await db.query(`UPDATE age_verifications SET status = 'verified', verified_dob = $1 WHERE stripe_session_id = $2`, [`${dob.year}-${String(dob.month).padStart(2, '0')}-${String(dob.day).padStart(2, '0')}`, vs.id]);
                }
                else {
    ...
    const orderSchema = zod_1.z.object({
        ...
        items: zod_1.z.array(zod_1.z.object({ id: zod_1.z.string(), quantity: zod_1.z.number().min(1) })).min(1),
        paymentMethodId: zod_1.z.string().optional(),
        name: zod_1.z.string().min(2).optional(),
        email: zod_1.z.string().email().optional(),
        age: zod_1.z.number().min(21).optional()
    });
    app.post('/api/checkout', async (request, reply) => {
        ...
        if (authUser && db) {
            const result = await db.query(`SELECT first_name, last_name, email, age_verified FROM users WHERE id = $1`, [authUser.userId]);
            if (result.rows[0]) {
                const u = result.rows[0];
        ...
        if (db && authUser) {
            ...
            await db.query(`INSERT INTO orders (id, user_id, cart_id, status, total) VALUES ($1, $2, $3, 'pending', $4)`, [orderId, authUser.userId, cartId, total]);
            if (cartId) {
